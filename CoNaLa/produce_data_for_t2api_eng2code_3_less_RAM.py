...
def get_dict_of_questions_and_code___not_stemmed_nor_tokenized(
        input_file,
        is_jsonl_binary_file=False,
        isForReferenceFile=False,
        is_train_not_tune=True):
...
#         text = shakes.read()
        ...
        if IS_IN_REVERSE___IE_CODE_2_ENGLISH:
            ...
            if is_train_not_tune:
                with open('train.ts', 'a+') as transl_answers_file:
                    ...
                    sentence = no_punctuation_WITH_SPACES_AT_END_FOR_DIFFERENTIATION.replace('\n', ' ').replace('\r', '')
                    transl_answers_file.write(sentence+"\n")
                with open('train.cd', 'a+') as transl_input_file:
                    ...
                    sentence = line_snippet_lower_WITH_SPACES_AT_END_FOR_DIFFERENTIATION.replace('\n', ' ').replace('\r', '')
                    transl_input_file.write(sentence+"\n")
            else:
                with open('tune.ts', 'a+') as transl_answers_file:
                    #json.dump(dict_of_questions_and_code_TRAIN.keys, outfile_results_conala_eval)
                    #transl_answers_file.write(dict_of_questions_and_code_TRAIN.values().totsring())
                    # for sentence in dict_of_questions_and_code_TEST.values():
                    sentence = no_punctuation_WITH_SPACES_AT_END_FOR_DIFFERENTIATION.replace('\n', ' ').replace('\r', '')
                    transl_answers_file.write(sentence+"\n")
                with open('tune.cd', 'a+') as transl_input_file:
                    #json.dump(dict_of_questions_and_code_TRAIN.values, transl_input_file)
                    #transl_input_file.write(dict_of_questions_and_code_TRAIN.keys().tostring())
...
dict_of_questions_and_code_TEST = get_dict_of_questions_and_code___not_stemmed_nor_tokenized(
    test_file,
    is_jsonl_binary_file=False,
    is_train_not_tune=False
)
