...
IS_FOR_JAVA_DOCSTRING_CORPUS = False
IS_FOR_JAVA_DOCSTRING_CORPUS_reversed = False

IS_FOR_PYTHON_DOCSTRING_CORPUS = True
IS_FOR_PYTHON_DOCSTRING_CORPUS_non_reposplit = False
IS_FOR_PYTHON_DOCSTRING_CORPUS_reversed = False
...
def stem_tokens(tokens, stemmer):
    if len(tokens) == 0 or tokens == "" or tokens is None:
        print("emtpy tokens")
        print(tokens)
    stemmed = []
    for item in tokens:
...
def tokenize(text):
    ...
    # # /CHANGE_NEW
    if len(stems) == 0 or stems is None or stems == "":
        print("stems empty")
        print(stems)
    return stems
...
if IS_FOR_JAVA_DOCSTRING_CORPUS:
    # NON REPO_SPLIT -->
    # input_file = "/Users/nicolasg-chausseau/funcom_processed/conala-java-train.json"
    input_file = "/Users/nicolasg-chausseau/funcom_processed/conala-java-train-REPOSPLIT.json" # TODO get that file from folder on macbook.
elif IS_FOR_JAVA_DOCSTRING_CORPUS_reversed:
    pass # todo: cimplement this
elif IS_FOR_PYTHON_DOCSTRING_CORPUS:
    # NON REPO_SPLIT --> input_file = "/Users/nicolasg-chausseau/python_docstring_corpus/conala-pydoc-train.json"
    input_file = "/Users/nicolasg-chausseau/python_docstring_corpus/conala-pydoc-train-REPOSPLIT.json"
else:
...
def return_data_as_dict_of_SOquestions_stemmed____to____original_sentences_____for_word2vec_input_format(dict_of_questions_and_code_2):
    counter_of_empty_english_sentences_after_tokenization = 0
    counter_of_NON_empty_english_sentences_after_tokenization = 0
    ...
    for key in dict_of_questions_and_code_2.keys():
        ...
        if not stemmed_and_tokenized or len(stemmed_and_tokenized) == 0 or stemmed_and_tokenized == [] or None in stemmed_and_tokenized:
            ...
            counter_of_empty_english_sentences_after_tokenization += 1
            #print("counter_of_empty_english_sentences_after_tokenization --> "+str(counter_of_empty_english_sentences_after_tokenization))
            #sys.exit()
            if IS_FOR_PYTHON_DOCSTRING_CORPUS or IS_FOR_PYTHON_DOCSTRING_CORPUS_reversed:
                continue # DONE it's just that some first lines of docstrings are junk, like \n chars etc --TODO handle case weird character or invisible character ... # TODO: why is python docstring hitting this empty string?
        else:
            counter_of_NON_empty_english_sentences_after_tokenization += 1
            #print("counter_of_NON_empty_english_sentences_after_tokenization = 0 --> "+str(counter_of_NON_empty_english_sentences_after_tokenization))
            # nice this case is way more common ... TODO: lookinto why we do lose so many entries to tokenization though ...
...
model = gensim.models.Word2Vec(
    ...
    size=800, # 150, 1000, no effect really, 50 brings 1 bleu point down
    #size=500,
...
def get_tfidf_weights_for_words_in_sentence(sentence):
    ...
    if not positive_func or positive_func is None:
        ...
        #sys.exit() # TODO: re-run java corpus with no sys.exit() and get new results (entire results).
    print(" ----- bis? ------")
...
def get_mean_for_sentence(positive):
    ...
    for word, weight in positive:# + negative:
        ...
        if isinstance(word, ndarray):
            ...
            try:
                ...
                import numpy as np
                x1 = np.array(w2vec_vector)
                np.linalg.norm(x1)
                w2vec_vector_NORMALIZED = x1
    ...
    if not mean:
        ...
        if not IS_FOR_PYTHON_DOCSTRING_CORPUS:
            sys.exit() # actually we should ignore new words, not set them to zero, which would skew the avg ... so abort in this case.
            # TODO: generally it's when the query or retrieved sentence (or both) are empty strings, or just spaces ...
    ...
    import numpy as np
    x1 = np.array(sentence_vector_mean)
    np.linalg.norm(x1)
    sentence_vector_mean_NORMALIZED = x1
...
for doc_stemmed, doc in dict_of_SOquestions_stemmed___to___original_SOquestions.items():  # , doc
    if not doc_stemmed or doc_stemmed == []:
        ...
        sys.exit() # TODO: why is python docstring hitting this empty string?
    i += 1
...
def get_BLEU_score_for_2_strings___NLTK_way(reference_answer_as_code, corresponding_code, is_BLEU_for_code_evaluation_NOT_matchedQuery=True):
    ...
    #print(score) # TODO: try bigrams --> tweakable param
...
def getBleuScoreForSearchQuery_AndAddResultToListOfResults(english_query, reference_answer_as_code, output_file_suffix, conala_suffix):
    ...
    if not sentence_to_be_avgd or len(sentence_to_be_avgd) == 0 or sentence_to_be_avgd == [] or None in sentence_to_be_avgd:
        ...
        #sys.exit() # TODO check if this happens for pydoc test set too but I assume so
    # dict_of___English___to___stemmed_and_tokenized_English.append(stemmed_and_tokenized)
    ...
    for indices_unwrapped in indices:
        for place in indices_unwrapped:
            try:
                ...
                if num_neighbours_you_want == 1:
                    # ----- nic change jul 2019: dont store in RAM, append to file directly: -----
                    # --- old way:
                    ...
                    # --- /old way:
                    # --- new way, appending:
                    with open('FACEBOOK_CODE_2_ENG'+str(output_file_suffix)+str(conala_suffix)+'.test.json', 'a+') as outfile_results_conala_eval:
                        json.dump(corresponding_code, outfile_results_conala_eval)
                        outfile_results_conala_eval.write(", \n")
                    with open('FACEBOOK_CODE_2_ENG'+str(output_file_suffix)+str(conala_suffix)+'.queries.json', 'a+') as outfile_QUERIES_conala_eval:
                        json.dump(english_query, outfile_QUERIES_conala_eval)
                        outfile_QUERIES_conala_eval.write(", \n")
                    # --- /new way, appending:
                    # ----- /nic change jul 2019: dont store in RAM, append to file directly: -----
                else:
...
output_file_suffix = ""
if IS_FOR_JAVA_DOCSTRING_CORPUS:
    input_file_REFERENCE_javadoc = "/Users/nicolasg-chausseau/funcom_processed/conala-java-test.json"
    ref_files = [input_file_REFERENCE_javadoc]
    output_file_suffix = "_javadoc"
elif IS_FOR_PYTHON_DOCSTRING_CORPUS_non_reposplit:
    input_file_REFERENCE_conala = "/Users/nicolasg-chausseau/Downloads/conala-corpus/conala-test.json"
    input_file_REFERENCE_pydoc = "/Users/nicolasg-chausseau/python_docstring_corpus/conala-pydoc-test.json"
    ref_files = [input_file_REFERENCE_conala, input_file_REFERENCE_pydoc]
    output_file_suffix = "_pydocNONREPOSPLIT"
elif IS_FOR_PYTHON_DOCSTRING_CORPUS:
    input_file_REFERENCE_conala = "/Users/nicolasg-chausseau/Downloads/conala-corpus/conala-test.json"
    # NOT REPO SPLIT --> input_file_REFERENCE_pydoc = "/Users/nicolasg-chausseau/python_docstring_corpus/conala-pydoc-test.json"
    input_file_REFERENCE_pydoc = "/Users/nicolasg-chausseau/python_docstring_corpus/conala-pydoc-test-REPOSPLIT.json"
    ref_files = [input_file_REFERENCE_conala, input_file_REFERENCE_pydoc]
    output_file_suffix = "_pydoc"
elif IS_FOR_PYTHON_DOCSTRING_CORPUS_reversed:
    output_file_suffix = "_pydoc_reversed" # TODO reversed
else:
    input_file_REFERENCE_conala = "/Users/nicolasg-chausseau/Downloads/conala-corpus/conala-test.json"
    ref_files = [input_file_REFERENCE_conala]
    output_file_suffix = "_pySO"


counter=0
conala_suffix = ""
for input_file_REFERENCE in ref_files:

    # ----- nic change jul 2019: dont store in RAM, append to file directly: -----
    # --- old way:
    # nothing was done here before.
    # --- /old way:
    # --- new way, appending:
    # prepare result files with bracket since we now append to them:
    if counter == 0:
        counter += 1
        conala_suffix = "_conala"
    else:
        counter += 1
        conala_suffix = ""
    outfile_results_conala_eval = open('FACEBOOK_CODE_2_ENG'+str(output_file_suffix)+str(conala_suffix)+'.test.json', 'w')
    outfile_results_conala_eval.write("[")
    outfile_results_conala_eval.close()
    outfile_results_conala_eval = open('FACEBOOK_CODE_2_ENG'+str(output_file_suffix)+str(conala_suffix)+'.queries.json', 'w')
    outfile_results_conala_eval.write("[")
    outfile_results_conala_eval.close()
    # --- /new way, appending:
    # ----- /nic change jul 2019: dont store in RAM, append to file directly: -----
    ...
    if not IS_FOR_JAVA_DOCSTRING_CORPUS and not IS_FOR_PYTHON_DOCSTRING_CORPUS:
    ...
    for SOquestion, code in dict_of_questions_and_code_REFERENCE.items():
        ...
        getBleuScoreForSearchQuery_AndAddResultToListOfResults(SOquestion, code, output_file_suffix, conala_suffix)
    ...
    # ----- nic change jul 2019: dont store in RAM, append to file directly: -----
    # --- old way:
    import json # if you actually complete the script, write the whole output I guess ... but at least you'll have a backup already written to disk
    # with open('FACEBOOK_CODE_2_ENG_'+str(output_file_suffix)+'.test.json', 'w') as outfile_results_conala_eval:
    #     json.dump(list_of_results_to_write_to_json_file_for_BLEU4_eval, outfile_results_conala_eval)
    # with open('FACEBOOK_CODE_2_ENG_'+str(output_file_suffix)+'.queries.json', 'w') as outfile_QUERIES_conala_eval:
    #     json.dump(list_of_QUERIES_to_write_to_json_file_for_BLEU4_eval, outfile_QUERIES_conala_eval)
    # --- /old way:
    # --- new way, appending:
    # --DONE TODO: add bracket at start and enf of file programmatically ... for now though just do it manually at end of run.
    outfile_results_conala_eval = open('FACEBOOK_CODE_2_ENG'+str(output_file_suffix)+str(conala_suffix)+'.test.json', 'a+')
    outfile_results_conala_eval.write("\n]")
    outfile_results_conala_eval.close()
    outfile_results_conala_eval = open('FACEBOOK_CODE_2_ENG'+str(output_file_suffix)+str(conala_suffix)+'.queries.json', 'a+')
    outfile_results_conala_eval.write("\n]")
    outfile_results_conala_eval.close() # TODO: if this code doesnt complete, to get BLEU / eval on a partial test set, manually add bracket at end.
# --- /new way, appending:
# ----- /nic change jul 2019: dont store in RAM, append to file directly: -----
