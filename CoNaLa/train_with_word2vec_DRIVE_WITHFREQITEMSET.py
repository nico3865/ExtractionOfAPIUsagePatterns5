...
IS_OUTPUTTING_INPUTS_FOR_QUERY_ENHANCING = True
IS_OUTPUTTING_FOR_CONALA_SEQ2SEQ = True
IS_OUTPUTTING_FOR_VANILLA_TRANSF = True

# TODO: remove numbers and variable names from English ...
# TODO: train word2vec including code tokens, too.
...
def tokenize(text):
    ...
    stems_filtered = stems
    # /CHANGE_OLD

    # # CHANGE_NEW
    # # don't stem words
    # stems = tokens # problem is it was stemming the words twice ... everytime you call fit_transform or transform!!!!! really stup-0-dee-stoops!
    # stems_filtered = stems
    # # /CHANGE_NEW

    # # CHANGE_NEW11111 --> brings from 15.62, 15.96 ..... to 13.13.86 or 14.03
    # stems_filtered = []
    # for token in stems:
    ...
    #     if token not in list_of_uninteresting_code_tokens:
    #         stems_filtered.append(token)
    # # /CHANGE_NEW11111

    return stems_filtered

#def appendOnlyIfInterestingToken(token):
list_of_uninteresting_code_tokens = ['a', 'b', 'c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z',
                                     '1','2','3','4','5','6','7','8','9','0',
                                     '(',')','[',']','{','}',
                                     '\\', '//', '|','/','?','>','<','#','%','*', '+', '-', '=', '==', ',', ';', ':', '"', '\'',
                                     '\'\'', '``', '`', '_', '\'_',
...
def tokenize_CODE_FOR_FREQ_ITEMSET(text):
    tokens = nltk.word_tokenize(text.lower())
    ...
    for token in tokens:
        ...
        if len(token_split) > 1:
            ...
            for new_token in token_split:
                if new_token not in list_of_uninteresting_code_tokens:
...
def get_dict_of_questions_and_code___not_stemmed_nor_tokenized(input_file, is_jsonl_binary_file=False, isForReferenceFile=False):
    ...
    #         text = shakes.read()
            ...
            # add id, to be able to replicate a train set for conala seq2seq:
            # TODO: this doesn't work (since it's a map not a list of json objects) but ids might not even be needed --> dict_of_questions_and_code["question_id"] = line["question_id"]
...
model = gensim.models.Word2Vec(
    ...
    size=200, # 150, 1000, no effect really, 50 brings 1 bleu point down
    #size=500,
...
def get_mean_for_sentence(positive):
    ...
    for word, weight in positive:# + negative:
        ...
        if isinstance(word, ndarray):
            ...
            try:
                w2vec_vector = model.wv.word_vec(word, use_norm=True)
                # print(w2vec_vector)
                ...
                print(e_1)
                print("word not in vocab? word = "+str(word))
                print(" --> "+str(e_1))
    ...
    if not mean:
        ...
        sys.exit() # actually ew should ignore new words, not set them to zero, which would skew the avg ... so abort in this case.
...
def get_BLEU_score_for_2_strings___CONALA_way(reference_answer_as_code, corresponding_code, is_BLEU_for_code_evaluation_NOT_matchedQuery=True):
    ...
    #     "intent": "null",
    #     "rewritten_intent" : "none",
    #     "snippet": reference_answer_as_code
...
list_of_QUERIES_CORRESPONDING_TO_HYP_to_write_to_json_file_for_BLEU4_eval = []
list_of_results_to_write_to_json_file_for_BLEU4_eval_FREQ_ITEMSET = []
def getBleuScoreForSearchQuery_AndAddResultToListOfResults(english_query, reference_answer_as_code, question_id=-1):
    ...
    num_neighbours_you_want = 30 #20 # @FREQ_PARAM: use smaller number? try it ....! might improve results
    nbrs = NearestNeighbors(n_neighbors=num_neighbours_you_want, leaf_size=1, algorithm='brute').fit(X)
    ...
    list_of_code_answers_to_test_question___for_freq_itemset = [] # freq itemset:
    list_of_ENGLISH_for_code_answers_to_test_question___for_freq_itemset = []
    for indices_unwrapped in indices:
        ...
        for place in indices_unwrapped:
            try:
                ...
                # ---- freq itemset: ---
                list_of_code_answers_to_test_question___for_freq_itemset.append(corresponding_code)
                list_of_ENGLISH_for_code_answers_to_test_question___for_freq_itemset.append(corresponding_original_SOquestion)
                # ---- / freq itemset ----
                ...
                if is_first_ie_best_result: #num_neighbours_you_want == 1:
                    ...
                    list_of_QUERIES_CORRESPONDING_TO_HYP_to_write_to_json_file_for_BLEU4_eval.append(english_query)
                ...
                print(e)
                #sys.exit() # TODO: happens with
                pass

    #----freq itemset: ------
    # https://spark.apache.org/docs/2.4.0/ml-frequent-pattern-mining.html
    ...
    #list_for_df = [(code_entry, tokenize(code_entry)) for code_entry in list_of_code_answers_to_test_question___for_freq_itemset]
    list_for_df = [(i, list(set(tokenize_CODE_FOR_FREQ_ITEMSET(code_entry)))) for i, code_entry in enumerate(list_of_code_answers_to_test_question___for_freq_itemset, 1)]
    ...
    #     (2, [1, 2])
    # ], ["id", "items"])
    df = sqlContext.createDataFrame(list_for_df, ["id", "items"])

    fpGrowth = FPGrowth(itemsCol="items", minSupport=0.1, minConfidence=0.1) # @FREQ_PARAM: # @FREQ_PARAM:
    ...
    print()
    print("just to remind, we're for query --> "+english_query)
    print("just to remind, the solution is --> "+reference_answer_as_code)

    # display most freq itemsets of size 10, then 9, then 8, and every time check their BLEU score to see if indeed bigger is better:
    best_candidates = []
    best_freq_itemset_of_each_size = []
    for i in range(8, 0, -1):  # @FREQ_PARAM: range(10, 0
        print("i --> "+str(i))
        filterdf_size_i = model.freqItemsets.filter(size('items') == i).sort(col("freq").desc())
        filterdf_size_i.show(40, False)
        best_list_of_API_elements = []
        best_freq_for_itemset = -1
        for row in filterdf_size_i.limit(5).toPandas().itertuples(): # @FREQ_PARAM: limit(1)
            print()
            print
            if best_freq_for_itemset == -1:
                best_freq_for_itemset = row.freq
                best_freq_itemset_of_each_size += row.items # for transformer query enhancing
            drop_in_freq = best_freq_for_itemset - row.freq
            # TODO: make this a percentage drop isntead of absolute drop ... but anyway might not want to use
            if best_freq_for_itemset != -1 and drop_in_freq < 1:  # @FREQ_PARAM: max for drop_in_freq
                best_list_of_API_elements = row.items
                #break
                ...
                # 					// idf_of_words
                # 					rank --> cutoff
                # 					// number_of_subset_itemsets * their_rank
                # size + freq + rank_of_supporting_answers + num_itemset_of_size(indicates low certainty when high)
                # or take the most picked answer across all sizes!!! that works well actually.
                #       --> as if each itemset size gets a vote ...
                #       --> size 1,2 and 3 are surprisingly robust, pick great answers, often same as facebook's
                # the question is: how to tell when there is one clear pattern emerging, or when not ...

                print(best_list_of_API_elements)
                if len(best_list_of_API_elements) > 0 and best_list_of_API_elements != [''] and best_list_of_API_elements!= [None] and best_list_of_API_elements is not None:
                    best_list_of_API_elements_JOINED = " ".join(best_list_of_API_elements)
                    print(best_list_of_API_elements_JOINED)
                    get_BLEU_score_for_2_strings___CONALA_way(reference_answer_as_code, best_list_of_API_elements_JOINED, is_BLEU_for_code_evaluation_NOT_matchedQuery=True)

                    # find highest ranked result that contains this freq itemset:
                    for answer in list_of_code_answers_to_test_question___for_freq_itemset:
                        if all(x in answer for x in best_list_of_API_elements):
                            print()
                            print("again, the reference code --> "+str(reference_answer_as_code))
                            best_answer_FROM_FREQ_ITEMSET = answer
                            # TODO: print best answer's english ....
                            # assuming they are in ranked order, then the first one is the best
                            print("best answer .... according to freq itemset --> "+str(best_answer_FROM_FREQ_ITEMSET))
                            print("TODO: best answer's English .... --> "+str(best_answer_FROM_FREQ_ITEMSET))
                            print("its bleu score: ")
                            get_BLEU_score_for_2_strings___CONALA_way(
                                reference_answer_as_code,
                                best_answer_FROM_FREQ_ITEMSET,
                                is_BLEU_for_code_evaluation_NOT_matchedQuery=True
                            )
                            print()
                            print("bleu score from best answer from Facebook: --> " +str(list_of_code_answers_to_test_question___for_freq_itemset[0]))
                            get_BLEU_score_for_2_strings___CONALA_way(
                                reference_answer_as_code,
                                list_of_code_answers_to_test_question___for_freq_itemset[0],
                                is_BLEU_for_code_evaluation_NOT_matchedQuery=True
                            )
                            best_candidates.append(best_answer_FROM_FREQ_ITEMSET)
                            break

    # get most popular answer that contains the most freq itemsets across all itemset sizes:
    from collections import Counter
    dict_of_candidates_counts = Counter(best_candidates)
    if len(dict_of_candidates_counts) > 0:
        print(dict_of_candidates_counts)
        best_candidate_FROM_FREQ_ITEMSET = dict_of_candidates_counts.most_common(1)[0][0]
        print(best_candidate_FROM_FREQ_ITEMSET)

        print("~~~~~~~~~~~ FINAL BEST ANSWER FROM FREQ ITEMSET ~~~~~~~~~~~~~")
        print("just to remind, we're for query --> "+english_query)
        print("just to remind, the solution is --> "+reference_answer_as_code)

        print("best answer .... according to freq itemset --> "+str(best_candidate_FROM_FREQ_ITEMSET))
        print("its bleu score: ")
        get_BLEU_score_for_2_strings___CONALA_way(
            reference_answer_as_code,
            best_candidate_FROM_FREQ_ITEMSET,
            is_BLEU_for_code_evaluation_NOT_matchedQuery=True
        )

        # append it for eval later with conala_eval.sh:
        list_of_results_to_write_to_json_file_for_BLEU4_eval_FREQ_ITEMSET.append(corresponding_code)
    else: # if there is no freq itemset answer, just default to facebook's best answer
        list_of_results_to_write_to_json_file_for_BLEU4_eval_FREQ_ITEMSET.append(list_of_code_answers_to_test_question___for_freq_itemset[0])
    ...
    # ---- append freq itemsets to english, for query enhancing for transformer on google colab: ------
    if IS_OUTPUTTING_INPUTS_FOR_QUERY_ENHANCING:
        print("just to remind, we're for query --> "+english_query)
        print("just to remind, the solution is --> "+reference_answer_as_code)
        print(best_freq_itemset_of_each_size)

        # ---- tokenize with old code: ----
        #from CoNaLa.train_with_word2vec import tokenize
        from CoNaLa.conala_eval import tokenize_for_bleu_eval
        reference_answer_as_code_TOKENIZED = tokenize_for_bleu_eval(reference_answer_as_code)
        # ---- /tokenize with old code ----

        if IS_OUTPUTTING_FOR_VANILLA_TRANSF:
            with open("encoder_inputs_for_transformer_WITH_QUERY_ENHANCING.json", "a+") as encoder_inputs:
                enhanced_query = english_query.split(" ") + ['~'] + list(set(best_freq_itemset_of_each_size))
                try:
                    enhanced_query.remove('')
                except:
                    pass
                #encoder_inputs.write(str(enhanced_query) + ", ") # invalid json unescaped ...
                json.dump(enhanced_query, encoder_inputs)
                encoder_inputs.write(", ")
                print("wrote to file, for transofmer encoder input --> "+str(enhanced_query))
            with open("decoder_inputs_for_transformer_WITH_QUERY_ENHANCING.json", "a+") as decoder_inputs:
                #decoder_inputs.write(str(reference_answer_as_code_TOKENIZED) + ", ") # invalid json ... unescaped
                json.dump(reference_answer_as_code_TOKENIZED, decoder_inputs)
                decoder_inputs.write(", ")
                # TODO: redo previous run with full conala-train.json ... I actually just trained on the test set ... 500 entries.
                print("wrote to file, for transofmer decoder input --> "+str(reference_answer_as_code_TOKENIZED))
        if IS_OUTPUTTING_FOR_CONALA_SEQ2SEQ:
            with open("conala-train-for_seq2seq_WITH_QUERY_ENHANCING.json", "a+") as encoder_inputs:
                enhanced_query = english_query.split(" ") + ['~'] + list(set(best_freq_itemset_of_each_size))
                try:
                    enhanced_query.remove('')
                except:
                    pass
                #encoder_inputs.write(str(enhanced_query) + ", ") # invalid json unescaped ...
                enhanced_as_string = " ".join(enhanced_query)
                json_object = {}
                json_object["intent"] = enhanced_as_string #english_query
                json_object["rewritten_intent"] = enhanced_as_string #english_query
                json_object["snippet"] = reference_answer_as_code
                # json_object["question_id"] = question_id
                json.dump(json_object, encoder_inputs)
                encoder_inputs.write(", ")
                print("wrote to file, for seq2seq encoder input --> "+str(json_object))
                # TODO: don't forget to add brackets at start and end, and remove last comma.
# ---- /append freq itemsets to english, for query enhancing for transformer on google colab: ------
    print("finished appending results for query enhancing with freq items.")
...
if IS_OUTPUTTING_INPUTS_FOR_QUERY_ENHANCING:
    input_file_REFERENCE = "/Users/nicolasg-chausseau/Downloads/conala-corpus/conala-train.json"
    #TODO: try mined later too --> input_file_REFERENCE = "/Users/nicolasg-chausseau/Downloads/conala-corpus/conala-mined.json"
else:
...
if not IS_OUTPUTTING_INPUTS_FOR_QUERY_ENHANCING: # since we run this script on the training set when running for enhanced queries production
...
for SOquestion, code in dict_of_questions_and_code_REFERENCE.items(): # TODO: --> , question_id
    ...
    getBleuScoreForSearchQuery_AndAddResultToListOfResults(SOquestion, code) # TODO: question_id --> , question_id=question_id)
...
with open('FACEBOOK_CODE_2_ENG.queries.json', 'w') as outfile_QUERIES_conala_eval:
    json.dump(list_of_QUERIES_CORRESPONDING_TO_HYP_to_write_to_json_file_for_BLEU4_eval, outfile_QUERIES_conala_eval)

# freq itemset:
with open('FACEBOOK_CODE_2_ENG_freq_itemset.test.json', 'w') as outfile_results_conala_eval:
    json.dump(list_of_results_to_write_to_json_file_for_BLEU4_eval_FREQ_ITEMSET, outfile_results_conala_eval)
